...
class BaseModel(torch.nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(
            m, Detect
        ) or isinstance(m, DetectConvNeXt):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect, YOLOEDetect, YOLOESegment
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, Detect) or isinstance(m, DetectConvNeXt):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, DetectConvNeXt, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, v10ATSSDetect, v10Detect_aux}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, DetectConvNeXt, YOLOEDetect, Segment, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
            ...
            c2 = ch[f[-1]]
        elif m is SubpixelUpsample:
            c1 = ch[f] 
            
            # MONAI SubpixelUpsample signature: (spatial_dims, in_channels, out_channels=None, scale_factor=2, ...)
            
            yaml_spatial_dims = args[0]
            yaml_out_channels_config = args[1]
            
            constructor_args = [yaml_spatial_dims, c1]  # [spatial_dims, in_channels]
            
            if yaml_out_channels_config is not None:
                out_channels_val = yaml_out_channels_config
                if out_channels_val != nc:
                    out_channels_val = make_divisible(min(out_channels_val, max_channels) * width, 8)
                constructor_args.append(out_channels_val)
                c2 = out_channels_val
            else: # out_channels is None in YAML config
                constructor_args.append(None) 

                c2 = c1 // (args[2] ** yaml_spatial_dims)
                
            constructor_args.extend(args[2:]) 
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect, DetectConvNeXt)):
                return "detect"
