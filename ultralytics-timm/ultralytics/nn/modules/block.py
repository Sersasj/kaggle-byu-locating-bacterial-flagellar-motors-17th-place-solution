...
class ConvNeXtBlock(nn.Module):
    """
    ConvNeXt block (Liu et al., 2022) as a drop-in replacement for standard conv modules.
    Uses depthwise conv, LayerNorm, pointwise convs, GELU and optional stochastic depth.
    """
...
#    """SPP-ELAN with corrected padding calculation."""
#
#    def __init__(self, c1, c2, c3, k=5):
#        super().__init__()
#        self.c = c3
#        self.cv1 = Conv(c1, c3, 1, 1)
#        
#        # Calculate padding to preserve spatial dimensions exactly
#        # For odd kernel sizes, padding = k//2 works
#        # But we need to ensure no size mismatch
#        padding = k // 2
#        
#        self.cv2 = nn.MaxPool2d(kernel_size=k, stride=1, padding=padding)
#        self.cv3 = nn.MaxPool2d(kernel_size=k, stride=1, padding=padding)
#        self.cv4 = nn.MaxPool2d(kernel_size=k, stride=1, padding=padding)
#        self.cv5 = Conv(4 * c3, c2, 1, 1)
#
#    def forward(self, x):
#        """Forward pass through SPPELAN layer."""
#        y = [self.cv1(x)]
#        
#        # Apply sequential max pooling
#        current = y[0]
#        for pool_layer in [self.cv2, self.cv3, self.cv4]:
#            current = pool_layer(current)
#            # Ensure spatial dimensions match the first tensor
#            if current.shape[2:] != y[0].shape[2:]:
#                current = F.interpolate(current, size=y[0].shape[2:], mode='nearest')
#            y.append(current)
#            
#        return self.cv5(torch.cat(y, 1))
...
class Timm(nn.Module):
    ...
    Args:
        ...
        split (bool, optional): Returns output from intermediate child modules as list. Default is False.
        drop_path_rate (float, optional): Dropout path rate. Default is 0.0.
    """

    def __init__(self, model, pretrained=True, unwrap=True, truncate=0, split=False, drop_path_rate=0.0):
        ...
        if unwrap:
            ...
            features = self.m(dummy_input)

            # Print the feature map shapes
            print("Feature shapes:")
            for i, f in enumerate(features):
                print(f"Feature {i}: {f.shape}")
        else:
            self.m = timm.create_model(model, features_only=False, pretrained=pretrained, drop_path_rate=drop_path_rate)
            self.unwrap = False
